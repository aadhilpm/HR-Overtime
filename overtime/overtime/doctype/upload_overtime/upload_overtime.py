# -*- coding: utf-8 -*-
# Copyright (c) 2018, info@progressiveit.in and contributors
...
def add_header(w):
	...
	w.writerow(["ID", "Employee", "Employee Name", "Date", "Status", "Leave Type", "Company", "Naming Series" ,"In Time", "Out Time","Overtime in Minutes","OT Rate"])
	return w
	...
	for date in dates:
		for employee in employees:
			...
			row = [
				...
				existing_attendance and existing_attendance.overtime_in_minutes or  "",
				existing_attendance and existing_attendance.ot_rate or  "",
			]
			w.writerow(row)
	return w
...
def upload():
	...
	for i, row in enumerate(rows[5:]):
		...
		if d.name:
			...
			d["docin_time"] = frappe.db.get_value("Attendance", d.name, "docin_time")
			d["docout_time"] = frappe.db.get_value("Attendance", d.name, "docout_time")
			d["docovertime_in_minutes"] = frappe.db.get_value("Attendance", d.name, "docovertime_in_minutes")
			d["docot_rate"] = frappe.db.get_value("Attendance", d.name, "docot_rate")
